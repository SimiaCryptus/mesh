// js/geometry.js
    ...
    export function faceGeom(P, F) {
      return F.map(([a, b, c]) => ({
        a: P[a], b: P[b], c: P[c], ia: a, ib: b, ic: c,
    ...
    export function signedClearance(p, faces) {
      ...
      for (const f of faces) {
        ...
        if (d < best) { best = d; sign = info.sign; }
      }
      return sign * best;
    }
    ...
    export function enforceClearance(p, faces, delta) {
      let best = Infinity, bestClose = null, bestSign = 1;
      for (const f of faces) {
        const info = pointTriInfo(p, f.a, f.b, f.c);
        const d = Math.sqrt(info.d2);
        if (d < best) { best = d; bestClose = info.close; bestSign = info.sign; }
      }
      ...
      let dn = norm(dir);
      if (dn < 1e-9) dir = normalize(p);           // p ~ on surface: fall back to radial
