// js/mesh-energy.js
    ...
    export class MeshEnergy {
      ...
      constructor(topo, opts = {}) {
        const F = topo.faces, E = topo.edges, C = topo.corners, V = topo.V;
        this.V = V;

        this.faceA = tf.tensor1d(F.map((f) => f[0]), "int32");
        this.faceB = tf.tensor1d(F.map((f) => f[1]), "int32");
        this.faceC = tf.tensor1d(F.map((f) => f[2]), "int32");
        this.edgeI = tf.tensor1d(E.map((e) => e[0]), "int32");
        this.edgeJ = tf.tensor1d(E.map((e) => e[1]), "int32");
      ...
      setWeights(w) { Object.assign(this.w, w); }
      setVstar(v) { this.Vstar = v; }
      ...
      _terms(P) {
        ...
        const kern = dd.square().div(-2 * this.h * this.h).exp();
        ...
        const cosT = uu.mul(vv).sum(1).div(nu.mul(nv)).clipByValue(-1 + EPS_SIN, 1 - EPS_SIN);
        const theta = cosT.acos();
        const ang = theta.sub(Math.PI / 3).square().sum();
      ...
      dispose() {
        [this.faceA, this.faceB, this.faceC, this.edgeI, this.edgeJ,
         this.cApex, this.cL, this.cR, this.Lmat, this.centers,
         this.targets, this.fidW].forEach((x) => x && x.dispose());
      }
    }